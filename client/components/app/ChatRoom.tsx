     ...
     <>
       ...
       {!open && (
         <button
           ...
           className="fixed right-6 bottom-6 z-50 rounded-full bg-primary px-4 py-3 text-sm font-medium text-primary-foreground shadow-lg"
         >
           Global Chat
         </button>
       )}
         ...
         <div className="fixed right-6 bottom-20 z-50 w-96 max-w-full rounded-lg border bg-white shadow-lg dark:bg-background">
           <div className="flex items-center justify-between border-b p-3">
             ...
             <div className="flex items-center gap-3 text-xs text-muted-foreground">
               ...
               <button onClick={() => setOpen(false)} className="rounded border px-2 py-0.5">Close</button>
             </div>
           </div>
           <div
             ref={boxRef}
             className="max-h-64 overflow-auto p-3 flex flex-col-reverse gap-2"
           >
             {messages.map((m) => (
               <div
                 key={m.id}
                 className={`flex ${m.from === (user?.name ?? "You") ? "justify-end" : "justify-start"}`}
               >
                 <div
                   className={`max-w-[80%] rounded-md p-2 ${m.from === (user?.name ?? "You") ? "bg-primary text-primary-foreground" : "bg-muted/50 text-muted-foreground"}`}
                 >
