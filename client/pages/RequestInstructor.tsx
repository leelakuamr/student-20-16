...
export default function RequestInstructor() {
  ...
  const [status, setStatus] = useState<"none" | "pending" | "instructor">("none");
  ...
  async function refresh() {
    ...
    try {
      ...
      else if ((data.requestStatus as string) === "pending" && (data.requestedRole as string) === "instructor") setStatus("pending");
      else setStatus("none");
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="mx-auto max-w-xl space-y-4">
      ...
      <p className="text-sm text-muted-foreground">Request instructor access. An admin will review and approve.</p>
      ...
      <div className="rounded-xl border p-4">
        {loading ? (
          ...
          <div className="space-y-3">
            ...
            <button onClick={cancel} className="rounded-md border px-3 py-1 text-sm">Cancel request</button>
          </div>
        ) : (
          <div className="space-y-3">
            <p className="text-sm">Not an instructor yet.</p>
            <button onClick={request} className="rounded-md bg-primary px-4 py-2 text-primary-foreground">Request Instructor Access</button>
          </div>
        )}
      </div>
    </div>
