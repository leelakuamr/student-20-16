  ...
  try {
    ...
    if (!isFirebaseAdminReady()) {
      return res
        ...
        .json({
          error:
            "Firebase Admin not configured. Set FIREBASE_SERVICE_ACCOUNT_JSON or FIREBASE_SERVICE_ACCOUNT_BASE64 env.",
        });
    }
    ...
    const adminRole =
      (adminSnap.exists ? (adminSnap.data() as any).role : undefined) ||
      (decoded as any).role;
    if (adminRole !== "admin")
